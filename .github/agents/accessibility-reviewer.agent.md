---
description: Reviews frontend accessibility, keyboard support, reduced motion, and live-region behavior
...
combat logic or visual styling choices that don't affect accessibility.
...
rule set, and `python tools/accessibility-check.py` as the deterministic
check this review should agree with — if the script would fail, say so.
